  ...
  return <div className={focusStyles.page} data-focus-active={Boolean(session)} data-app-hero data-hero-theme="dark">
    ...
    {session ? <section aria-label={t("Current focus")} className={focusStyles.session}>
      ...
      <div className={`${styles.actions} ${focusStyles.sessionActions}`}>
        ...
        {session.taskId && target.data && !target.data.isCompleted && !target.data.deletedAtUtc && <Button variant="success" loading={action.isPending} onClick={() => action.mutate({ path: `/focus-sessions/${session.id}/stop`, body: { outcome: 'Completed', completeTask: true } })}>{t("Complete task & finish")}</Button>}
      </div>
