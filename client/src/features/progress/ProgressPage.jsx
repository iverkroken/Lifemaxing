...
export function ProgressSummary({ compact = false }) {
  ...
  return <section aria-label={t("Your progression")} className={compact ? progressStyles.compact : progressStyles.summary}>
    ...
    {data && <>{!compact && <RankBadge rank={data.rank} />}<div className={styles.sectionHeading}><h2>{t('levelNumber', { level: data.level })} <span className={styles.meta}>· {t(data.rank)}</span></h2><Link to="/progress">{number(data.totalXp)} XP</Link></div>
...
export function ActivityPage() {
  ...
  return <div className={styles.stack}><PageHeader eyebrow={t("Your record")} title={t("Activity")} description={t("What you did, and the corrections along the way.")} />
    ...
    </Select><QueryFeedback query={query} /><section className={styles.surface} aria-label={t("Activity history")}><ActivityList data={query.data} /></section><Pagination data={query.data} setPage={setPage} /></div>
}
...
export function ProgressPage() {
  ...
  return <div className={styles.stack}><PageHeader eyebrow={t("Completion and progress")} title={t("Progress")} description={t("Your earned XP, completed work and recorded focus, with the history behind them.")} action={<Link to="/rewards">{t("Your rewards →")}</Link>} />
    <ProgressSummary />
