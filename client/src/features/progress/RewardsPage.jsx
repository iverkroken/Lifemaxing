...
export function RewardsPage() {
  ...
  return <div className={styles.stack}><PageHeader eyebrow={t("Something to look forward to")} title={t("Rewards")} description={t("Personal incentives, chosen by you.")} action={<Button onClick={() => setCreating(true)}>{t("New reward")}</Button>} />
    ...
    {query.data?.total === 0 && <EmptyState title={t("Make room for something good")}>{t("Define a reward and the level that makes it available.")}</EmptyState>}
    <div className={styles.tileGrid}>{query.data?.items.map(reward => <section className={`${styles.tile} ${rewardStyles.reward}`} key={reward.id} aria-label={reward.title} data-state={reward.claimedAtUtc ? 'claimed' : reward.eligible ? 'ready' : 'locked'}>
      <div className={rewardStyles.identity}><Icon name={reward.claimedAtUtc ? 'check' : 'rewards'} size={40} /><span className={styles.badge}>{reward.claimedAtUtc ? t("Claimed") : reward.eligible ? t("Ready to claim") : t('levelNumber', { level: number(reward.requiredLevel) })}</span></div><h2>{reward.title}</h2>
      ...
      {!reward.archivedAtUtc && <div className={styles.actions}>{!reward.claimedAtUtc && <><Button disabled={!reward.eligible} loading={action.isPending} onClick={() => action.mutate({ path: `/rewards/${reward.id}/claim` })}>{t("Claim reward")}</Button><Button variant="quiet" onClick={() => setEditing(reward)}>{t("Edit reward")}</Button></>}
