...
export function GoalsPage() {
  ...
  return <div className={`${styles.stack} ${pageStyles.page}`}>
    ...
    <div className={pageStyles.goalList}>{goals.data?.items.map(goal => <article className={pageStyles.goal} key={goal.id}>
      <div className={pageStyles.goalIdentity}>
      <div className={styles.sectionHeading}><AreaLabel area={areas.data?.find(area => area.id === goal.lifeAreaId)} /><StatusBadge tone={!goal.archivedAtUtc && goal.state === "Completed" ? "success" : "neutral"}>{goal.archivedAtUtc ? t("Archived") : t(goal.state)}</StatusBadge></div>
      <h2><Link to={`/goals/${goal.id}`}>{goal.title}</Link></h2>
      {goal.description && <p className={pageStyles.description}>{goal.description}</p>}
      <span className={styles.meta}>{goal.targetDate ? t('targetDateValue', { date: date(goal.targetDate) }) : t("At your own pace")}</span>
      </div>
      <div className={pageStyles.goalEvidence}><GoalProgress goal={goal} /><Link className={pageStyles.progressLink} to={`/goals/${goal.id}`}>{t("View progress →")}</Link></div>
...
export function GoalDetailPage() {
  ...
  return <div className={styles.stack}>
    ...
    {data && <div className={pageStyles.detail}>
      ...
      <section className={pageStyles.update} aria-label={t("Record progress")}><h2 className={styles.sectionTitle}>{t("Record an update")}</h2>
