...
export function QuickAdd({ date }) {
  ...
  return <Card aria-label="Quick Add">
    ...
    <form className={styles.form} onSubmit={form.handleSubmit(values => action.mutate({ path: '/tasks', body: values }))} noValidate>
      ...
      <Input label="Task title" placeholder="What needs doing?" required error={form.formState.errors.title?.message} {...form.register('title')} />
