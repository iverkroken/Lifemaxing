...
export function TasksPage({ inbox = false }) {
  ...
  return <div className={`${styles.stack} ${pageStyles.workspace}`} data-inbox={inbox} data-scoped={Boolean(scopedArea)}>
    {scopedArea ? <header className={styles.sectionHeading}><h2>{t('Tasks')}</h2><Button onClick={() => openCapture({ date: null })}>{t('Capture a task')}</Button></header> : <PageHeader title={inbox ? t("Inbox") : t("Tasks")}
    ...
    <div className={pageStyles.results}>
      <section className={pageStyles.work} aria-label={t("Task workspace")}>
        <div className={pageStyles.toolbar}>
          ...
          {!scopedArea && <Select label={t("Life Area filter")} value={areaId} onChange={e => changeFilter('areaId', e.target.value)}>
            <option value="">{t("All areas")}</option>{areas.data?.map(area => <option key={area.id} value={area.id}>{areaName(area)}</option>)}
          </Select>}
          {(search || (!scopedArea && areaId) || status !== 'active') && <Button variant="quiet" onClick={() => setParams({}, { replace: true })}>{t("Reset filters")}</Button>}
        </div>
        <QueryFeedback query={areas} />{inbox && <QueryFeedback query={today} />}<QueryFeedback query={tasks} /><ActionFeedback action={action} />
        {tasks.data && <div className={pageStyles.resultHeading}>
          <p className={pageStyles.count} aria-live="polite">{t('taskCount', { count: tasks.data.total, status: t(inbox ? 'unplanned' : { active: 'Active', completed: 'Completed', all: 'All unarchived', archived: 'Archived' }[status]) })}{!inbox && <> · {t('All dates')}</>}</p>
          {!inbox && <Link to="/today">{t('Daily plan')} →</Link>}
        </div>}
        {tasks.data?.items.length === 0 && <EmptyState className={pageStyles.empty} title={search || areaId ? t("No matching tasks") : inbox ? t("Your Inbox is clear") : t("No tasks in this view")} icon={inbox ? 'inbox' : 'tasks'} action={<Button variant="secondary" onClick={() => openCapture({ date: null })}>{t("Capture a task")}</Button>}>{inbox ? t("Your Inbox is clear. Capture an idea whenever it comes to mind.") : t("Create your next action, or adjust the filters to find earlier work.")}</EmptyState>}
        <ul className={styles.list}>{tasks.data?.items.map(task => <TaskRow key={task.id} task={task} action={action}
          date={inbox ? today.data?.currentLocalDate : undefined} area={areas.data?.find(area => area.id === task.lifeAreaId)} />)}</ul>
        <Pagination data={tasks.data} setPage={setPage} />
      </section>
      <div className={styles.actions}><Link to={scopedArea ? `/tasks/new?areaId=${scopedArea.id}` : '/tasks/new'}>{t("Create task with details →")}</Link>{inbox && <p className={styles.meta}>{t("Choose a planned date to move a task out of Inbox.")}</p>}</div>
    </div>
  </div>
}
