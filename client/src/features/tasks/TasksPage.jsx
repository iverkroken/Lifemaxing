...
export function TasksPage({ inbox = false }) {
  ...
  return <div className={`${styles.stack} ${pageStyles.workspace}`} data-inbox={inbox}>
    <PageHeader title={inbox ? t("Inbox") : t("Tasks")}
    ...
    <div className={pageStyles.results}>
      <section className={pageStyles.work} aria-label={t("Task workspace")}>
        ...
        {tasks.data && <p className={pageStyles.count} aria-live="polite">{t('taskCount', { count: tasks.data.total, status: t(inbox ? 'unplanned' : { active: 'Active', completed: 'Completed', all: 'All unarchived', archived: 'Archived' }[status]) })}</p>}
