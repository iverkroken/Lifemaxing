...
test('plans and completes real tasks, habits and goals across responsive screens', async ({ page }) => {
  ...
  await page.getByRole('combobox', { name: 'Daily priority' }).selectOption({ label: 'Prepare weekly plan carefully' })
  const todayTasks = page.getByRole('region', { name: 'Tasks Today', exact: true })
  await expect(todayTasks.getByRole('link', { name: 'Prepare weekly plan carefully' })).toBeVisible()
  await todayTasks.getByRole('checkbox', { name: 'Complete Prepare weekly plan carefully' }).click()
  await expect(todayTasks.getByRole('checkbox', { name: 'Reopen Prepare weekly plan carefully' })).toBeChecked()
  await todayTasks.getByRole('checkbox', { name: 'Reopen Prepare weekly plan carefully' }).click()
  await expect(todayTasks.getByRole('checkbox', { name: 'Complete Prepare weekly plan carefully' })).not.toBeChecked()
  await todayTasks.getByRole('checkbox', { name: 'Complete Prepare weekly plan carefully' }).click()
  await expect(todayTasks.getByRole('checkbox', { name: 'Reopen Prepare weekly plan carefully' })).toBeChecked()
  ...
  const progressHistory = page.getByRole('region', { name: 'Progress history', exact: true })
  await expect(progressHistory.getByText('First reading update', { exact: true })).toBeVisible()
  await page.getByLabel('Current value (chapters)').fill('1')
  await page.getByLabel('Progress note').fill('Corrected reading update')
  await page.getByRole('button', { name: 'Record progress' }).click()
  await expect(progressHistory.getByText('Corrected reading update', { exact: true })).toBeVisible()
  await page.reload()
  await expect(progressHistory.getByText('First reading update', { exact: true })).toBeVisible()
  await expect(progressHistory.getByText('Corrected reading update', { exact: true })).toBeVisible()
  ...
  await mobileTask.getByLabel('Task actions').click()
  await mobileTask.getByRole('button', { name: 'Cancel plan' }).click()
  await page.getByText(/Planning history/).click()
  await expect(mobileTask.getByText('Cancelled', { exact: false })).toBeVisible()
  await mobileTask.getByRole('button', { name: 'Plan for this day' }).click()
  await mobileTask.getByLabel('Task actions').click()
