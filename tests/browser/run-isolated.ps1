param([ValidateSet('', 'before', 'after')][string] $ReferenceStage = '', [switch] $UxRefresh, [switch] $Redesign, [switch] $SignatureShell, [switch] $Artwork, [switch] $Quality, [switch] $SelectedFeatures, [switch] $AreaDetails,
  ...
  [string] $ArtifactRoot = '', [switch] $Headed, [string] $TestFilter = '', [switch] $DailyProgression, [switch] $DeletionRestore, [switch] $FocusHub,
...
try {
  ...
  if ($PerformanceStage -or $Redesign -or $SignatureShell -or $Artwork -or $Quality -or $SelectedFeatures -or $AreaDetails -or $DailyProgression -or $DeletionRestore -or $FocusHub) { $viteArguments = 'node_modules/vite/bin/vite.js preview client --config client/vite.config.js --port 5174 --strictPort' }
  ...
  if ($FocusHub) {
    ...
    $browserArguments = @('run', 'test:smoke', '--', 'focus-hub.spec.js')
    if ($TestFilter) { $browserArguments += @('--grep', $TestFilter) }
    if ($Headed) { $browserArguments += '--headed' }
    & npm @browserArguments
    if ($LASTEXITCODE -ne 0) { throw 'Focus hub checks failed.' }
    return
  }
  if ($DeletionRestore) {
    $env:SMOKE_BROWSER = $BrowserEngine
