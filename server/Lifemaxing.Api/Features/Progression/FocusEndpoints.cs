...
public sealed record FocusStart(Guid? TaskId);
...
public sealed record FocusResponse(Guid Id, Guid? TaskId, string Status, DateTimeOffset StartedAtUtc, DateTimeOffset? RunningSinceUtc,
    long AccumulatedSeconds, long ElapsedSeconds, DateTimeOffset? EndedAtUtc, DateTimeOffset ServerNow);
...
public static class FocusEndpoints
{
    public static void MapFocusEndpoints(this RouteGroupBuilder api)
    {
        ...
        api.MapPost("/focus-sessions", async (FocusStart request, ClaimsPrincipal principal, AppDbContext db, TimeProvider clock, CancellationToken ct) =>
        {
            ...
            var session = new FocusSession { Id = Guid.NewGuid(), UserId = owner, TaskId = request.TaskId, StartedAtUtc = now, RunningSinceUtc = now };
    ...
    private static async Task<IResult> Change(Guid id, string operation, FocusStop? request, Guid owner, AppDbContext db, TimeProvider clock, CancellationToken ct)
    {
        ...
        else
        {
            ...
            var title = session.TaskId.HasValue ? await db.Tasks.Where(x => x.Id == session.TaskId && x.UserId == owner).Select(x => x.Title).SingleAsync(ct) : "Unstructured focus";
    ...
    private static FocusResponse Response(FocusSession x, DateTimeOffset now) => new(x.Id, x.TaskId, x.Status, x.StartedAtUtc, x.RunningSinceUtc, x.AccumulatedSeconds, Elapsed(x, now), x.EndedAtUtc, now);
}
